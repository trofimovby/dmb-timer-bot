...
class TestFindParentPackage:
    def test_single_package(self, tmp_path):
        ...
        (tmp_path / "src/namespace/pkg/nested/init_db.py").touch()
        (tmp_path / "src/namespace/pkg/init_db.py").touch()
    ...
    def test_multiple_toplevel(self, tmp_path):
        ...
        for name in multiple:
            ...
            (tmp_path / f"src/{name}/init_db.py").touch()
...
class TestDiscoverPackagesAndPyModules:
    ...
    FILES = {
        "src": ["src/pkg/init_db.py", "src/pkg/main.py"],
        "lib": ["lib/pkg/init_db.py", "lib/pkg/main.py"],
        "flat": ["pkg/init_db.py", "pkg/main.py"],
        "single_module": ["pkg.py"],
        "namespace": ["ns/pkg/init_db.py"],
    }
    ...
    def test_project(self, tmp_path, circumstance):
        ...
        (tmp_path / "build/lib/init_db.py").touch()
    ...
    def test_purposefully_empty(self, tmp_path, config_file, param, circumstance):
        files = self.FILES[circumstance] + ["mod.py", "other.py", "src/pkg/init_db.py"]
...
class TestNoConfig:
    ...
    EXAMPLES = {
        "pkg1": ["src/pkg1.py"],
        "pkg2": ["src/pkg2/init_db.py"],
        "pkg3": ["src/pkg3/init_db.py", "src/pkg3-stubs/init_db.py"],
        "pkg4": ["pkg4/init_db.py", "pkg4-stubs/init_db.py"],
        "ns.nested.pkg1": ["src/ns/nested/pkg1/init_db.py"],
        "ns.nested.pkg2": ["ns/nested/pkg2/init_db.py"],
    }
    ...
    def test_build_with_discovered_name(self, tmp_path):
        files = ["src/ns/nested/pkg/init_db.py"]
        _populate_project_dir(tmp_path, files, {})
...
class TestWithAttrDirective:
    ...
    def test_pyproject_metadata(self, tmp_path):
        _populate_project_dir(tmp_path, ["src/pkg/init_db.py"], {})

        overwrite = {
            "src": {"pkg": {"init_db.py": "version = 42"}},
            "pyproject.toml": (
                "[project]\nname = 'pkg'\ndynamic = ['version']\n"
