...
def test_read_only(tmpdir_cwd):
    ...
    os.makedirs('pkg')
    open('pkg/init_db.py', 'wb').close()
    open('pkg/data.dat', 'wb').close()
    os.chmod('pkg/init_db.py', stat.S_IREAD)
...
def test_excluded_subpackages(tmpdir_cwd):
    ...
    assert (build_dir / "mypkg/init_db.py").exists()
...
def test_existing_egg_info(tmpdir_cwd, monkeypatch):
    ...
    assert outputs
    example = str(Path(build_py.build_lib, "mypkg/init_db.py")).replace(os.sep, "/")
...
def test_get_outputs(tmpdir_cwd):
    ...
    assert outputs == {
        f"{build_lib}/mypkg/init_db.py",
        f"{build_lib}/mypkg/resource_file.txt",
        f"{build_lib}/mypkg/sub1/init_db.py",
        f"{build_lib}/mypkg/sub1/mod1.py",
        f"{build_lib}/mypkg/sub2/mod2.py",
        f"{build_lib}/mypkg/sub2/nested/init_db.py",
